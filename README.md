...
| `pub-10/` | Spectral analysis ("Grams"). `dita/` source only — it has no template of its own. Published twice from `pub-5/template-2026/`: the full edition to `site/pub-10/current/`, and the redacted one to `site/pub-9/current/` via an `audience` filter. The map title carries both edition names on the same `audience` axis, so it publishes as "Pub-10" or "Pub-9" without editing |
